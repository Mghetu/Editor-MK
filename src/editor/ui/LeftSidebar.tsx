...
export function LeftSidebar() {
  ...
  return (
    <div className={`grid h-full border-r border-[#232c45] bg-[#121a2f] ${panelOpen ? "grid-cols-[74px_1fr]" : "grid-cols-[74px]"}`}>
      <div className="border-r border-[#232c45] p-2">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          return (
            <button
              key={tab.key}
              className={`mb-2 flex w-full flex-col items-center rounded px-1 py-2 text-[10px] ${activeTab === tab.key ? "bg-[#253151] text-violet-300" : "hover:bg-[#1b2540] text-slate-300"}`}
      ...
      {panelOpen && (
        <div className="flex h-full min-w-[286px] flex-col">
          <div className="flex items-center justify-between border-b border-[#232c45] px-3 py-2 text-xs font-medium text-slate-200">
            <div className="flex items-center gap-2"><AppWindow size={14} /> {tabs.find((t) => t.key === activeTab)?.label}</div>
            <button className="rounded p-1 hover:bg-[#1b2540]" onClick={() => setTab("select")}><X size={14} /></button>
          </div>
