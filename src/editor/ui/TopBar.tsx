...
export function TopBar({ undo, redo, canUndo = true, canRedo = true, persistNow }: { undo: () => void; redo: () => void; canUndo?: boolean; canRedo?: boolean; persistNow?: () => void }) {
  ...
  return (
    <div className="flex h-14 items-center gap-2 border-b border-[#313131] bg-[#1f1f1f] px-4 text-slate-200">
      ...
      <button className="rounded p-2 hover:bg-[#2a2a2a] disabled:cursor-not-allowed disabled:opacity-40" onClick={undo} title="Undo" disabled={!canUndo}><Undo2 size={16} /></button>
